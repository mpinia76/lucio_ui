...
class LucioUISetup {
	 ...
	 */
	public static function initialize( $appName = "lucio_ui"){
...
//		$_SESSION['LAST_ACTIVITY'] = time();
	...
	private static function initializeCore(){
		...
		LucioConfig::getInstance()->setDbName("cose_lucio");

		LucioConfig::getInstance()->initialize();
		LucioConfig::getInstance()->initLogger(dirname(__DIR__) . "/conf/log4php.xml");
	...
	private static function initializeUI( $appName ){
		...
		Logger::log("Logger lucio ui configurado!");
		...
		$rastyLoader->loadXml(
				...
				RastyConfig::getInstance()->getAppPath() . "src/main/php/Lucio/UI/",
		...
		//SorteosUIConfig::getInstance()->initialize("LucioMetroLayout");
