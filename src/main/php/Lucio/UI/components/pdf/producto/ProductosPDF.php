...
namespace Lucio\UI\components\pdf\producto;
...
class ProductosPDF extends RastyComponent{
	...
	protected function parseXTemplate(XTemplate $xtpl){
		...
		$xtpl->assign( "fecha", LucioUIUtils::formatDateTimeToView(new Datetime()) );
		...
		foreach ($tipos as $tipo) {
			...
			foreach ($productos as $producto) {
				...
				$xtpl->assign( "precio", LucioUIUtils::formatMontoToView( $producto->getPrecioEfectivo() ) );
			...
			foreach ($arrTipos as $arrTipo) {
				...
				foreach ($packs as $pack) {
					if ($arrTipo->getOid()==$pack->getProducto()->getTipoProducto()->getOid()) {
						$html.='<tr><td>'.$pack->getProducto()->getMarcaProducto().' '.$pack->getProducto()->getNombre().' '.$pack->getNombre().'</td><td style="text-align: right;">'.$pack->getCantidad().'</td><td style="text-align: right;">'.LucioUIUtils::formatMontoToView($pack->getPrecioEfectivo()).'</td></tr>';
					}
			...
			if ($mostrarCombo) {
				...
				<hr />
								...
								foreach ($arrayCombos as $combo) {
									$productos='(';
									foreach ($combo->getProductos() as $value) {
										$productos.= $value->getCantidad(). ' '.$value->getProducto()->getNombre().' '.$value->getProducto()->getMarcaProducto().' '.LucioUIUtils::formatMontoToView($value->getPrecioUnitario()).'-';
									}
									$productos.=')';
										$html.='<tr><td>'.$combo->getNombre().' '.$productos.'</td><td style="text-align: right;">'.LucioUIUtils::formatMontoToView( $combo->getPrecio() ).'</td></tr>';
