...
class AdminHome extends LucioPage{
	...
	public function __construct(){
		...
		/*if( LucioUIUtils::isCajaSelected() ){
			$caja = UIServiceFactory::getUICajaService()->get(LucioUIUtils::getCaja()->getOid());
			$this->setCaja( $caja );
		}*/
	...
	public function parseLinks( XTemplate $xtpl){
        ...
        $xtpl->assign("menu_companias", $this->localize("menu.companias") );
        $xtpl->assign("linkCompanias", $this->getLinkCompanias() );
        $xtpl->assign("menu_companias_agregar", $this->localize("menu.companias.agregar") );
        $xtpl->assign("linkCompaniaAgregar", $this->getLinkCompaniaAgregar() );
	...
	public function parseStatsVentas(XTemplate $xtpl){
		...
		$xtpl->assign("monto_ventas", LucioUIUtils::formatMontoToView($totalesDia["monto"]) );
		...
		$xtpl->assign("monto_ventas_mes", LucioUIUtils::formatMontoToView($totalesMes["monto"]) );
	...
	public function parseDebitosCreditos( XTemplate $xtpl){
		...
		foreach ($debitosCreditos as $informeDebitoCredito) {
			...
			$xtpl->assign("titulo", LucioUIUtils::formatDateToView( $informeDebitoCredito->getFechaVencimiento()) );

			if($informeDebitoCredito->getDebito()){
				$xtpl->assign("subtitulo", LucioUIUtils::formatMontoToView($informeDebitoCredito->getDebito()) );
				$xtpl->assign("descripcion", "Débito" );
			}else{
				$xtpl->assign("subtitulo", LucioUIUtils::formatMontoToView($informeDebitoCredito->getCredito()) );
	...
	public function parseSaldos(XTemplate $xtpl){
		...
		//if( LucioUIUtils::isCajaSelected() ){
			...
			$xtpl->assign("saldo_caja", LucioUIUtils::formatMontoToView( UIServiceFactory::getUIMovimientoCajaService()->getTotalesDia(new Datetime())) );
			$xtpl->assign("saldo_ventas", LucioUIUtils::formatMontoToView( UIServiceFactory::getUIMovimientoVentaService()->getTotalesDia(new Datetime())) );
			$xtpl->assign("saldo_gastos", LucioUIUtils::formatMontoToView( UIServiceFactory::getUIMovimientoGastoService()->getTotalesDia(new Datetime())) );

			$xtpl->assign("saldo_cajaTarjeta", LucioUIUtils::formatMontoToView( UIServiceFactory::getUIMovimientoCajaService()->getTotalesTarjetasDia(new Datetime())) );
			$xtpl->assign("saldo_cajaCtaCte", LucioUIUtils::formatMontoToView( UIServiceFactory::getUIMovimientoCajaService()->getTotalesCtasCtesDia(new Datetime())) );
		...
		/*}else{
			$xtpl->assign("saldo_caja", LucioUIUtils::formatMontoToView( 0) );
		}*/

		/*$xtpl->assign("saldo_cajaChica", LucioUIUtils::formatMontoToView( $this->getCajaChica()->getSaldo() ) );
		$xtpl->assign("linkMovimientosCajaChica", $this->getLinkMovimientosCajaChica());

		$xtpl->assign("saldo_bancos", LucioUIUtils::formatMontoToView( UIServiceFactory::getUIBancoService()->getSaldoBancos() ) );
		$xtpl->assign("linkMovimientosBanco", $this->getLinkMovimientosBanco());

		$xtpl->assign("saldo_ctasctes", LucioUIUtils::formatMontoToView( UIServiceFactory::getUICuentaCorrienteService()->getSaldoCtasCtes() ) );
	*/
	}
	...
	public function parseGastos( XTemplate $xtpl){
		...
		foreach ($gastos as $gasto) {
			$xtpl->assign("titulo", LucioUIUtils::formatDateToView( $gasto->getFechaVencimiento()) );
			$xtpl->assign("subtitulo", LucioUIUtils::formatMontoToView($gasto->getMonto()) );
